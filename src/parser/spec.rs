...
/// Macro to implement common numeric visitor methods that convert to string
macro_rules! impl_numeric_visitors {
    ($wrapper:expr) => {
    ...
    D: Deserializer<'de>,
{
    ...
    impl<'de> Visitor<'de> for OptionalStringVisitor {
        ...
        impl_numeric_visitors!(|s| Ok(Some(s)));
